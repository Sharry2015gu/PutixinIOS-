...
- (void)viewDidLoad {
    ...
    [self getScribeRanking:_model IsFresh:YES];
...
-(void)createTableView
{
    ...
    [tableview addHeaderWithCallback:^{
        [weakself getScribeRanking:_model IsFresh:YES];
    }];
    [tableview addFooterWithCallback:^{
        [weakself getScribeRanking:_model IsFresh:NO];
    }];
    CGFloat width;
    CGFloat height ;
    if (fabs(SCREEN_HEIGHT-667) < 1) {
    ...
    NSInteger userOrder=[[appDelegate.accountBasicDict objectForKey:@"writeUserOrder"]integerValue];
...
-(void)getScribeRanking:(CollectModel *)model IsFresh:(BOOL)fresh{
    ...
    NSDictionary * dic = @{@"secretKey":miyaoNum,@"taskCode":@501,@"bookID":[NSString stringWithFormat:@"%ld",(long)model.objectID],@"orderType":@"T",@"pageIndex":[NSNumber numberWithInteger:page]};
...
- (void)loadPersonWrite:(CollectModel*)model
{
    ...
    [appDelegate.sessionManager GET:[NSString stringWithFormat:@"%@%@",__kBaseURLString,ptxGetBookOrderByUserID] parameters:rankingDic success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        NSLog(@"用户抄经排行=%@",jsonDate);
        NSNumber *order =jsonDate[@"userOrder"];
        [appDelegate.accountBasicDict setObject:order forKey:@"writeUserOrder"];
