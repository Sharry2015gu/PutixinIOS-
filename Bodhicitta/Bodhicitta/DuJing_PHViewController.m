...
- (void)viewDidLoad {
    ...
    [self getReadRankingWit:_model IsFresh:YES];
...
-(void)createTableView
{
    ...
    tableview = [[UITableView alloc] initWithFrame:CGRectMake(0,NAVBARHEIGHT*0.1+110,SCREEN_WIDTH,SCREEN_HEIGHT*0.79)];
    ...
    [tableview addHeaderWithCallback:^{
        [weakself getReadRankingWit:self.model IsFresh:YES];
    }];
    [tableview addFooterWithCallback:^{
        [weakself getReadRankingWit:_model IsFresh:NO
         ];
    }];
    ...
    NSInteger userOrder = [[appDelegate.accountBasicDict   objectForKey:@"readUserOrder"] integerValue];
    if (userOrder==0){
...
-(void)getReadRankingWit:(CollectModel *) model IsFresh:(BOOL)fresh
{
    ...
    NSDictionary * dic = @{@"secretKey":miyaoNum,@"taskCode":@500,@"bookID":[NSString stringWithFormat:@"%ld",(long)model.objectID],@"orderType":@"T",@"pageIndex":[NSNumber numberWithInteger:page]};
...
- (void)loadPersonread:(CollectModel*)model
{
    ...
    [appDelegate.sessionManager GET:[NSString stringWithFormat:@"%@%@",__kBaseURLString,ptxGetBookOrderByUserID] parameters:rankingDic success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        [appDelegate.accountBasicDict setObject:order forKey:@"readUserOrder"];
